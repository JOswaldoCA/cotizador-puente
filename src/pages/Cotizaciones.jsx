...
import {
  obtenerCotizaciones, eliminarCotizacion, obtenerCotizacion,
  actualizarEstatus, obtenerSeguimiento,
...
const inputCls = "border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 transition-all duration-200";
...
export default function Cotizaciones() {
  ...
  const enviarCorreo = async () => {
    ...
    const dest = email.split(";").map((e) => e.trim()).filter(Boolean);
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/enviar-cotizacion`,
        {
          ...
          body: JSON.stringify({ folio: emailModal.folio, email: dest, pdfBase64 }),
        }
      );
  ...
  const guardarEstatus = async () => {
    ...
    try {
      await actualizarEstatus(seguimientoModal.folio, nuevoEstatus, notaEstatus, perfil);
      setCotizaciones((prev) =>
        prev.map((c) => c.folio === seguimientoModal.folio
          ? { ...c, estatus: nuevoEstatus, notas_seguimiento: notaEstatus } : c)
      );
  ...
  const filtradas = cotizaciones.filter((c) => {
    ...
    const matchSucursal = filtroSucursal ? c.sucursal?.tipo === filtroSucursal : true;
    let matchFecha = true;
    if (fechaDesde || fechaHasta) {
      const partes = c.fecha?.toLowerCase().split(" de ");
      if (partes?.length === 3) {
        const fechaCot = new Date(parseInt(partes[2]), meses[partes[1]], parseInt(partes[0]));
        if (fechaDesde) matchFecha = matchFecha && fechaCot >= new Date(fechaDesde);
        if (fechaHasta) matchFecha = matchFecha && fechaCot <= new Date(fechaHasta);
      }
    }
    ...
    <>
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl lg:text-2xl font-bold text-primary-600 tracking-tight">
              Historial
            </h2>
          ...
          <div className="flex items-center gap-2">
            ...
            {(fechaDesde || fechaHasta) && (
              <button onClick={() => { setFechaDesde(""); setFechaHasta(""); }}
                className="text-xs text-red-400 hover:text-red-600 font-medium px-2 py-1 rounded-lg hover:bg-red-50 transition-colors whitespace-nowrap">
                × Limpiar
              </button>
            )}
          </div>
                  ...
                  <div key={cot.folio}
                    ...
                    <div className="px-4 py-3 flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {vencida && (
                          <span className="text-xs bg-red-50 text-red-500 font-medium px-2 py-1 rounded-lg border border-red-200">
                            ⚠️ Vencida
                          </span>
                        )}
                      </div>
                    ...
                    <div className="px-4 pb-4 grid grid-cols-5 gap-2">
                      ...
                      {puedeEditar(cot) ? (
                        <button onClick={() => navigate(`/cotizaciones/${cot.folio}/editar`)}
                          className="col-span-1 text-xs py-2.5 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 text-center">
                          ✏️
                        </button>
                      ) : <div />}
                      <button onClick={() => abrirEmailModal(cot)}
                        className="col-span-1 text-xs py-2.5 rounded-xl border border-primary-100 text-primary-600 hover:bg-primary-50 text-center">
                        ✉️
                      </button>
                      <button onClick={() => abrirSeguimiento(cot)}
                        className="col-span-1 text-xs py-2.5 rounded-xl border border-amber-200 text-amber-600 hover:bg-amber-50 text-center">
                        📋
                      </button>
                      {puedeEditar(cot) ? (
                        <button onClick={() => eliminar(cot.folio)}
                          className="col-span-1 text-xs py-2.5 rounded-xl border border-red-100 text-red-400 hover:bg-red-50 text-center">
                          🗑️
                        </button>
                      ) : <div />}
                    </div>
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ background: 'linear-gradient(135deg, #1B3A6B 0%, #0F2347 100%)' }}>
                    ...
                    <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Opciones</th>
                    <th className="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Estatus</th>
                    <th className="px-5 py-3.5 text-right text-xs font-bold uppercase tracking-wider text-blue-200">Acciones</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={cot.folio}
                        className={`hover:bg-primary-50/30 transition-colors duration-150 ${
                          vencida ? "bg-red-50/60" : i % 2 === 0 ? "bg-white" : "bg-gray-50/30"
                        }`}>
                        ...
                        <td className="px-5 py-4 text-gray-500 text-xs">{cot.fecha}</td>
                        <td className="px-5 py-4">
                          ...
                          <div className="flex flex-col gap-1">
                            {cot.opciones?.map((op, j) => (
                              <div key={j} className="text-xs text-gray-500">
                                <span className="font-semibold text-primary-600">Op.{j + 1}</span>{" "}
                                <span className="font-mono">${op.total?.toLocaleString("es-MX", { minimumFractionDigits: 2 })}</span>
                              </div>
                            ))}
                          </div>
                        </td>
            ...
            <div className="p-6 flex flex-col gap-4">
              {enviado ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-4 text-center">
                  ...
                  <p className="text-sm font-bold text-emerald-700">¡Correo enviado!</p>
                  <p className="text-xs text-emerald-600 mt-1">
                    {email.split(";").map((e) => e.trim()).filter(Boolean).join(", ")}
                  </p>
                </div>
              ) : (
                <>
                  <div>
                    ...
                    <input type="text" placeholder="correo1@mail.com; correo2@mail.com"
                      value={email} onChange={(e) => setEmail(e.target.value)}
                      className={inputCls + " w-full"} />
                    <p className="text-xs text-gray-400 mt-1">Separa múltiples correos con punto y coma</p>
                  </div>
                  {errorEmail && (
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 px-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-md flex flex-col overflow-hidden"
            ...
            <div className="p-6 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
              ...
              {puedeEditar(seguimientoModal) && (
                <div className="flex flex-col gap-3 border border-gray-200 rounded-xl p-4"
                  ...
                  <textarea placeholder="Nota de seguimiento (opcional)..."
                    value={notaEstatus} onChange={(e) => setNotaEstatus(e.target.value)}
              ...
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Historial</p>
                {cargandoHist ? (
                      ...
                      <div key={h.id} className="border border-gray-100 rounded-xl px-4 py-3 bg-white"
                        ...
                        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg ${badgeEstatus(h.estatus_nuevo)}`}>
                              {h.estatus_nuevo}
                            </span>
                          </div>
                        ...
                        {h.nota && (
                          <p className="text-xs text-gray-600 bg-gray-50 rounded-lg px-3 py-2 mt-1">"{h.nota}"</p>
                        )}
                        <p className="text-xs text-gray-400 mt-1.5">
                          por <span className="font-medium text-gray-500">{h.nombre_usuario || "Usuario"}</span>
                        </p>
                      </div>
                    ))}
                  </div>
            ...
            <div className="px-6 pb-6 pt-2 border-t border-gray-100">
              <button onClick={() => setSeguimientoModal(null)}

...
const sucursalColor = (id) => {
  if (id === 'matriz')  return { bg: '#EEF2FF', border: '#C7D2FE', text: '#3730A3', icon: '🏙️' }
  if (id === 'guaymas') return { bg: '#ECFDF5', border: '#A7F3D0', text: '#065F46', icon: '⚓' }
  if (id === 'obregon') return { bg: '#FFF7ED', border: '#FED7AA', text: '#9A3412', icon: '🌵' }
...
export default function Usuarios() {
  ...
  const cambiarRol = async (id, rolActual) => {
    ...
    const { error } = await supabase
      .from('perfiles').update({ rol: nuevoRol }).eq('id', id)
    if (!error) setUsuarios(prev => prev.map(u => u.id === id ? { ...u, rol: nuevoRol } : u))
    setCambiando(null)
  }
  ...
  const activos = usuarios.filter(u =>
    u.last_sign_in_at && (AHORA - new Date(u.last_sign_in_at).getTime()) < 7 * 86400000
  ).length
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400">
          ...
          <span>›</span>
          <span className="text-primary-600 font-semibold">Gestión de usuarios</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 lg:gap-4">
        {[
          {
            icon: '👥', label: 'Total usuarios', value: usuarios.length,
            bg: 'linear-gradient(135deg, #1B3A6B, #0F2347)',
            textColor: 'text-white', subColor: 'text-blue-300',
            shadow: '0 4px 16px rgba(27,58,107,0.3)',
          },
          {
            icon: '⭐', label: 'Administradores', value: admins,
            bg: 'linear-gradient(135deg, #FFFBEB, #FEF3C7)',
            border: '#FDE68A', textColor: 'text-amber-800', subColor: 'text-amber-600',
            shadow: '0 2px 8px rgba(217,119,6,0.15)',
          },
          {
            icon: '🟢', label: 'Activos (7 días)', value: activos,
            bg: 'linear-gradient(135deg, #ECFDF5, #D1FAE5)',
            border: '#A7F3D0', textColor: 'text-emerald-800', subColor: 'text-emerald-600',
            shadow: '0 2px 8px rgba(16,185,129,0.15)',
          },
        ].map(({ icon, label, value, bg, border, textColor, subColor, shadow }) => (
          <div key={label} className="rounded-2xl p-4 lg:p-5 border flex flex-col gap-2"
            style={{ background: bg, borderColor: border || 'transparent', boxShadow: shadow }}>
      ...
      <div className="flex flex-col gap-3 lg:hidden">
        {filtrados.length === 0 ? (
          ...
          return (
            <div key={u.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden transition-all duration-200 hover:shadow-md"
              style={{ boxShadow: '0 2px 8px rgba(27,58,107,0.06)' }}>

              {/* Header */}
              <div className="px-4 py-4 flex items-center gap-3"
                style={{ background: 'linear-gradient(135deg, #1B3A6B 0%, #0F2347 100%)' }}>
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0 relative"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {u.id === perfil?.id && (
                      <span className="text-xs bg-white/20 text-white px-1.5 py-0.5 rounded-md font-medium flex-shrink-0">Tú</span>
                    )}
                  </div>
                  <p className="text-xs text-blue-300/70 truncate mt-0.5">{u.email}</p>
                </div>
              ...
              <div className="px-4 py-3 flex flex-col gap-2">
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-gray-50 rounded-xl px-3 py-2 border border-gray-100">
                    ...
                    <span className="text-xs font-bold px-2 py-0.5 rounded-lg border inline-block"
                      ...
                      {sc.icon} {SUCURSALES_LABEL[u.sucursal_id] || u.sucursal_id}
                    </span>
                  </div>
                  <div className="bg-gray-50 rounded-xl px-3 py-2 border border-gray-100">
                    <p className="text-xs text-gray-400 mb-1">Último login</p>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${rec ? 'bg-green-400' : 'bg-gray-300'}`}></div>
                      <span className="text-xs font-semibold text-gray-700">{tiempoDesde(u.last_sign_in_at)}</span>
                    </div>
                  </div>
                </div>

                <div className="bg-gray-50 rounded-xl px-3 py-2 border border-gray-100">
                  <p className="text-xs text-gray-400 mb-0.5">Registrado el</p>
                  <p className="text-xs font-medium text-gray-600">{formatFecha(u.created_at)}</p>
                </div>
                ...
                {u.id !== perfil?.id ? (
                  <button onClick={() => cambiarRol(u.id, u.rol)} disabled={cambiando === u.id}
                    ...
                    {cambiando === u.id
                      ? <><div className="w-3 h-3 border-2 border-current/30 border-t-current rounded-full animate-spin"></div>Cambiando...</>
      ...
      <div className="hidden lg:block bg-white rounded-2xl border border-gray-100 overflow-hidden"
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-primary-600">Directorio de usuarios</p>
              <p className="text-xs text-gray-400">{filtrados.length} de {usuarios.length} usuarios</p>
            </div>
          </div>
          {busqueda && (
            <button onClick={() => setBusqueda('')}
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: 'linear-gradient(135deg, #1B3A6B 0%, #0F2347 100%)' }}>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-amber-300">Usuario</th>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Sucursal</th>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Rol</th>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Último acceso</th>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-blue-200">Registrado</th>
              <th className="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-blue-200">Acciones</th>
            </tr>
          </thead>
            ...
            {filtrados.map((u, i) => {
              ...
              return (
                <tr key={u.id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {u.id === perfil?.id && (
                            <span className="text-xs bg-primary-50 text-primary-600 px-1.5 py-0.5 rounded-md border border-primary-100 font-semibold">Tú</span>
                          )}
                        </div>
                  ...
                  {/* Sucursal */}
                  <td className="px-6 py-4">
                    <span className="text-xs font-bold px-2.5 py-1.5 rounded-lg border inline-flex items-center gap-1"
                      style={{ background: sc.bg, borderColor: sc.border, color: sc.text }}>
                      {sc.icon} {SUCURSALES_LABEL[u.sucursal_id] || u.sucursal_id}
                    </span>
                  </td>
                  ...
                  {/* Rol */}
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-2.5 py-1.5 rounded-lg inline-flex items-center gap-1 ${
                      u.rol === 'admin'
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${rec ? 'bg-green-400' : 'bg-gray-200'}`}
                        style={rec ? { boxShadow: '0 0 6px rgba(74,222,128,0.6)' } : {}}></div>
                      <div>
                        <p className="text-xs font-semibold text-gray-700">{tiempoDesde(u.last_sign_in_at)}</p>
                        {u.last_sign_in_at && (
                          <p className="text-xs text-gray-400">{formatFecha(u.last_sign_in_at)}</p>
                        )}
                      </div>
                    </div>
                  </td>
                  ...
                  {/* Registrado */}
                  <td className="px-6 py-4">
                    <p className="text-xs font-medium text-gray-500">{formatFecha(u.created_at)}</p>
                  </td>
        ...
        {filtrados.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="text-sm font-semibold text-gray-500 mb-1">No se encontraron usuarios</p>

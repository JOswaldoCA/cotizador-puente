...
import { obtenerCotizacion, guardarCotizacion, actualizarCotizacion } from "../services/cotizaciones";
import { obtenerClientes } from "../services/sqlserver";
import { fechaHoy, calcularServicio, calcularTotalesOpcion, estaVencida } from "../utils/calculos";
...
const SectionHeader = ({ title, icon }) => (
  <div className="px-6 py-4 border-b border-white/10 flex items-center gap-3"
    ...
    <h2 className="text-xs font-bold text-amber-300 uppercase tracking-widest">{title}</h2>
  </div>
);
...
export default function EditarCotizacion() {
  ...
  const {
    folio, sucursal, opciones,
    agregarOpcion, quitarOpcion,
    agregarServicio, quitarServicio, actualizarServicio,
    cliente, setCliente,
    cargarCotizacion,
  ...
  const clientesFiltrados = clientes.filter(c =>
    c.razon_social?.trim().toLowerCase().includes(busquedaCliente.toLowerCase())
  ).slice(0, 10)
  ...
  const alSeleccionarServicio = (opId, srvId, nombre) => {
    ...
    actualizarServicio(opId, srvId, { servicioId: nombre, nombre, precioUnitario: srv?.precio_lista || 0 });
  };
  ...
  const guardar = async () => {
    ...
    setGuardando(true);
    try {
      const opcionesConTotales = opciones.map((op) => ({ ...op, ...calcularTotalesOpcion(op.servicios) }));
      if (esVencida) {
        await guardarCotizacion({ folio, fecha: fechaHoy(), sucursal, cliente, opciones: opcionesConTotales, basesExtra });
      } else {
        await actualizarCotizacion({ folio: folioParam, sucursal, cliente, opciones: opcionesConTotales, basesExtra });
      }
    ...
    <>
      <div className="max-w-3xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight" style={{ color: "#1B3A6B" }}>
              {esVencida ? "🔄 Renovar cotización" : "✏️ Editar cotización"}
            </h2>
            <p className="text-xs text-gray-400 font-mono mt-0.5">
              {esVencida ? `Nueva desde: ${folioParam}` : `Folio: ${folioParam}`}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold px-3 py-1.5 rounded-xl border"
              style={{ background: "rgba(255,215,0,0.1)", borderColor: "rgba(255,215,0,0.3)", color: "#92700a" }}>
              📅 {fechaHoy()}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-2xl px-5 py-4 border"
          style={{ background: "linear-gradient(135deg, #EEF2FF, #DBEAFE)", borderColor: "rgba(27,58,107,0.15)", boxShadow: "0 1px 3px rgba(27,58,107,0.06)" }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
            style={{ background: "linear-gradient(135deg, #1B3A6B, #0F2347)" }}>🏢</div>
          <div>
            <p className="text-sm font-bold text-primary-600">{sucursal?.tipo}</p>
            ...
            <div ref={buscadorRef}>
              ...
              <div className="relative">
                ...
                <input type="text"
                  ...
                  onFocus={() => { if (busquedaCliente) setMostrarLista(true); }}
                ...
                {busquedaCliente && (
                  <button type="button"
                    onClick={() => {
                      ...
                      setCliente(p => ({ ...p, atencion: '', contacto: '', email: '' }))
                    }}
                ...
                {mostrarLista && busquedaCliente && (
                  <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl overflow-hidden max-h-60 overflow-y-auto"
                    ...
                    {clientesFiltrados.length > 0 ? (
                      clientesFiltrados.map((c, i) => (
                        <button key={i} type="button"
                          ...
                          <p className="text-sm font-semibold text-gray-800">{c.razon_social?.trim()}</p>
                          {c.e_mail && <p className="text-xs text-gray-400 mt-0.5 truncate">{c.e_mail}</p>}
                        </button>
                      ))
                    ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select className={input} value={cliente.vigencia}
                  ...
                  {["1 MES", "15 DÍAS", "30 DÍAS"].map((v) => <option key={v}>{v}</option>)}
                </select>
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea className={input} rows={3} value={cliente.notas || ""}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"
          ...
          <div className="p-6 flex flex-col gap-5">
            {opciones.map((op, opIdx) => {
              const { subtotal, iva, total } = calcularTotalesOpcion(op.servicios);
              return (
                <div key={op.id} className="border border-gray-200 rounded-2xl overflow-hidden"
                  ...
                  <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-primary-600 tracking-wide">OPCIÓN {opIdx + 1}</span>
                    </div>
                    {opciones.length > 1 && (
                        ...
                        <div key={srv.id} className="p-5">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            {op.servicios.length > 1 && (
                              <button type="button" onClick={() => quitarServicio(op.id, srv.id)}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="col-span-2">
                              <label className={label}>Concepto de servicio</label>
                              <select className={inputSm} value={srv.nombre || ""}
                                onChange={(e) => alSeleccionarServicio(op.id, srv.id, e.target.value)}>
                            ...
                            <div>
                              ...
                              <input className={inputSm} type="number" min="0" step="0.01"
                                value={srv.precioUnitario || ""}
                                onChange={(e) => cambiarCampo(op.id, srv.id, "precioUnitario", e.target.value)} />
                            </div>
                            <div>
                              <label className={label}>N° contenedores</label>
                              <div className="flex items-center gap-1.5">
                                <button type="button" className={btnCounter}
                                  onClick={() => cambiarCampo(op.id, srv.id, "numContenedores", Math.max(1, srv.numContenedores - 1))}>−</button>
                                <input className={inputSm + " text-center font-semibold"} type="number" min="1"
                                  value={srv.numContenedores}
                                  onChange={(e) => cambiarCampo(op.id, srv.id, "numContenedores", e.target.value)} />
                                <button type="button" className={btnCounter}
                                  onClick={() => cambiarCampo(op.id, srv.id, "numContenedores", srv.numContenedores + 1)}>+</button>
                              </div>
                            </div>
                            <div>
                              ...
                              <input className={inputSm} type="number" min="0" step="0.01"
                                value={srv.precioDia || ""}
                                onChange={(e) => cambiarCampo(op.id, srv.id, "precioDia", e.target.value)} />
                            </div>
                            <div>
                              <label className={label}>Visitas por semana</label>
                              <div className="flex items-center gap-1.5">
                                <button type="button" className={btnCounter}
                                  onClick={() => cambiarCampo(op.id, srv.id, "diasSemana", Math.max(1, srv.diasSemana - 1))}>−</button>
                                <input className={inputSm + " text-center font-semibold"} type="number" min="1" max="7"
                                  value={srv.diasSemana}
                                  onChange={(e) => cambiarCampo(op.id, srv.id, "diasSemana", e.target.value)} />
                                <button type="button" className={btnCounter}
                                  onClick={() => cambiarCampo(op.id, srv.id, "diasSemana", Math.min(7, srv.diasSemana + 1))}>+</button>
                              </div>
                            </div>
                            <div className="col-span-2">
                              <div className="flex items-center justify-end gap-2 bg-gray-50 rounded-xl px-4 py-2.5 border border-gray-100">
                                <span className="text-xs text-gray-400 font-medium">Subtotal línea:</span>
                  ...
                  <div className="px-5 pb-4">
                    <button type="button" onClick={() => agregarServicio(op.id)}
                  ...
                  <div className="grid grid-cols-3 divide-x divide-gray-100 border-t border-gray-100">
                    {[
                      ...
                      <div key={l} className="text-center py-4 px-2"
                        style={highlight ? { background: "linear-gradient(135deg, #EEF2FF, #DBEAFE)" } : {}}>
                        <p className="text-xs text-gray-400 font-medium mb-1">{l}</p>
                        <p className={`text-sm font-bold font-mono ${highlight ? "text-primary-600" : "text-gray-700"}`}>
                          ${(v || 0).toLocaleString("es-MX", { minimumFractionDigits: 2 })}
                        </p>
                      </div>
                    ))}
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"
          ...
          <div className="p-6">
            <BasesEditor basesExtra={basesExtra} setBasesExtra={setBasesExtra} />
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button type="button" onClick={() => navigate("/cotizaciones")}
            className="flex-1 border border-gray-200 text-gray-600 font-semibold text-sm py-4 rounded-2xl hover:bg-gray-50 transition-colors">
            ← Cancelar
          </button>
          <button type="button" onClick={guardar} disabled={guardando}
            className="flex-1 text-white font-bold text-sm py-4 rounded-2xl transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2"
            style={{
              background: guardando ? "#6B7280" : "linear-gradient(135deg, #1B3A6B 0%, #0F2347 100%)",
            ...
            ) : esVencida ? "📋 Generar nueva cotización" : "💾 Guardar cambios"}
          </button>
        </div>
      </div>

    ...
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {BASES.map((b, i) => (
            <div key={i} className="flex items-start gap-3 text-xs text-gray-500 rounded-xl px-3 py-2.5 border border-gray-100"
              style={{ background: '#F8FAFF' }}>
              <span className="text-gray-300 mt-0.5 flex-shrink-0 font-bold">•</span>
          ...
          <div className="flex flex-col gap-1.5">
            {basesExtra.map((b, i) => (
              <div key={i}
                ...
                <span className="flex-1 text-primary-700 leading-relaxed font-medium">{b}</span>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={agregar}
          className="text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all duration-200 whitespace-nowrap"
      ...
      {basesExtra.length > 0 && (
        <p className="text-xs text-gray-400 text-center">
          {basesExtra.length} base{basesExtra.length !== 1 ? 's' : ''} personalizada{basesExtra.length !== 1 ? 's' : ''} agregada{basesExtra.length !== 1 ? 's' : ''}
        </p>
      )}
    </div>
  )
